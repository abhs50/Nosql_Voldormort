...
# $5 is the number of servlets servers
 ...
 if [ -d $1 ]; then
  ...
  for ((i = 0; i < $4; i = i + 1)); do
    ...
    awk -f bench/format_sar_output.awk $1db_server$i
    gzip -9 $1db_server$i &
  done
   gunzip $1"web_server.gz"
   awk -f bench/format_sar_output.awk $1"web_server"
   gzip -9 $1"web_server" &
  if [ -f $1"cjdbc_server.gz" ]; then
      gunzip $1"cjdbc_server.gz"
  fi
  if [ -f $1"cjdbc_server" ]; then
      awk -f bench/format_sar_output.awk $1"cjdbc_server"
  ...
  for ((i = 0; i < $5; i = i + 1)); do
    ...
    awk -f bench/format_sar_output.awk $1servlets_server$i
    gzip -9 $1servlets_server$i &
  done
   for ((i = 0 ; i < $3 ; i = i + 1)); do
     gunzip $1client$i.gz
     awk -f bench/format_sar_output.awk $1client$i
    gzip -9 $1client$i &
  done
