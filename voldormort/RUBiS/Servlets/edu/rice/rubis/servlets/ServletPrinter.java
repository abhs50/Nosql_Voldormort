...
public class ServletPrinter {
	...
	void printHTMLfooter() {
		...
		out
				.println("<br><hr>RUBiS (C) Rice University/INRIA<br><i>Page generated by "
						+ servletName
						+ " in "
	...
	void printCategory(String categoryName, int categoryId) {
		try {
			out
					.println("<a href=\"edu.rice.rubis.servlets.SearchItemsByCategory?category="
							+ categoryId
							+ "&categoryName="
	...
	void printCategoryByRegion(String categoryName, int categoryId, int regionId) {
		try {
			out
					.println("<a href=\"edu.rice.rubis.servlets.SearchItemsByRegion?category="
							+ categoryId
							+ "&categoryName="
							+ URLEncoder.encode(categoryName)
							+ "&region="
	...
	void printCategoryToSellItem(String categoryName, int categoryId, int userId) {
		try {
			out
					.println("<a href=\"edu.rice.rubis.servlets.SellItemForm?category="
							+ categoryId
							+ "&user="
							+ userId
							+ "\">"
	...
	void printRegion(String regionName) {
		try {
			out
					.println("<a href=\"edu.rice.rubis.servlets.BrowseCategories?region="
	...
	void printItem(String itemName, int itemId, float maxBid, int nbOfBids,
		...
		try {
			out
					.println("<TR><TD><a href=\"edu.rice.rubis.servlets.ViewItem?itemId="
							+ itemId
							+ "\">"
							+ itemName
							+ "<TD>"
	...
	void printItemDescription(int itemId, String itemName, String description,
		...
		try {
			...
			if (maxBid == 0) {
				...
				if (quantity > 1) {
					try {
						...
						Versioned v = client.get((Integer.toString(itemId)));
						HashMap<String, Object> item = (HashMap<String, Object>)v.getValue();
						ArrayList listOfBids = (ArrayList) item.get("bids");

						/*
						 * stmt = client.prepareStatement(
						 * "SELECT id,qty,max_bid FROM bids WHERE item_id=? ORDER BY bid DESC LIMIT ?"
						 * ); stmt.setInt(1, itemId); stmt.setInt(2, quantity);
						 * ResultSet rs = stmt.executeQuery();
						 */
						if (!listOfBids.isEmpty()) {
							int numberOfItems = 0;
							int qty;
							for (int k = 1; k < listOfBids.size(); k++) {
								HashMap<String, Object> bid = (HashMap<String, Object>) listOfBids
										.get(k);
								qty = (Integer) bid.get("qty");
								numberOfItems = numberOfItems + qty;
								if (numberOfItems >= quantity) {
									maxBid = (Float) bid.get("maxBid");
						...
						printHTML("Problem while computing current bid: " + e
								+ "<br>");
						return;
					}
				}
			...
			out
					.println("<TR><TD>Quantity<TD><b><BIG>"
							+ quantity
							+ "</BIG></b>\n"
							...
							+ nbOfBids
							+ "</BIG></b> (<a href=\"edu.rice.rubis.servlets.ViewBidHistory?itemId="
							+ itemId
							+ "\">bid history</a>)\n"
							+ "<TR><TD>Seller<TD><a href=\"edu.rice.rubis.servlets.ViewUserInfo?userId="
							+ sellerId
							+ "\">"
							+ sellerName
							+ "</a> (<a href=\"edu.rice.rubis.servlets.PutCommentAuth?to="
			...
			if (buyNow > 0)
				out
						.println("<p><a href=\"edu.rice.rubis.servlets.BuyNowAuth?itemId="
								+ itemId
								+ "\">"
			...
			if (userId <= 0) {
				out
						.println("<a href=\"edu.rice.rubis.servlets.PutBidAuth?itemId="
								+ itemId
			...
			if (userId > 0) {
				...
				printHTML("<form action=\"edu.rice.rubis.servlets.StoreBid\" method=POST>\n"
	...
	void printUserBidsHeader() {
		...
		out
				.println("<TABLE border=\"1\" summary=\"Items You've bid on\">\n"
						+ "<THEAD>\n"
		...
		try {
			out
					.println("<TR><TD><a href=\"edu.rice.rubis.servlets.ViewItem?itemId="
							...
							+ "<TD>"
							+ endDate
							+ "<TD><a href=\"edu.rice.rubis.servlets.ViewUserInfo?userId="
							+ sellerId
							+ "\">"
							+ sellerName
							+ "<TD><a href=\"edu.rice.rubis.servlets.PutBid?itemId="
							+ itemId
							+ "&nickname="
							+ username
							+ "&password="
	...
	void printUserBoughtItemHeader() {
		...
		out
				.println("<TABLE border=\"1\" summary=\"List of items\">\n"
						+ "<THEAD>\n"
		...
		try {
			out
					.println("<TR><TD><a href=\"edu.rice.rubis.servlets.ViewItem?itemId="
							...
							+ buyNow
							+ "\n"
							+ "<TD><a href=\"edu.rice.rubis.beans.servlets.ViewUserInfo?userId="
							+ sellerId + "\">" + sellerName + "</a>\n");
		} catch (Exception e) {
			out.println("Unable to print Item (exception: " + e + ")<br>\n");
		}
	...
	void printItemDescriptionToBuyNow(int itemId, String itemName,
		...
		try {
			...
			printHTML("<form action=\"edu.rice.rubis.servlets.StoreBuyNow\" method=POST>\n"
					+ "<input type=hidden name=userId value="
					+ userId
					+ ">\n"
					+ "<input type=hidden name=itemId value="
	...
	void printBidHistory(int userId, String bidderName, float bid, String date) {
		try {
			out
					.println("<TR><TD><a href=\"edu.rice.rubis.servlets.ViewUserInfo?userId="
							+ userId
							+ "\">"
							+ bidderName
							+ "<TD>"
	...
	void printComment(String userName, int userId, String date, String comment) {
		try {
			out
					.println("<DT><b><BIG><a href=\"edu.rice.rubis.servlets.ViewUserInfo?userId="
							+ userId
							+ "\">"
							+ userName
							+ "</a></BIG></b>"
