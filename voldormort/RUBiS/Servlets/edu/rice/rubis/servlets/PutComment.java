    ...
    throws IOException, ServletException
  {
    ...
    try
    {
      ...
      try
      {
    	  ...
    	  Versioned vc = client.get(toStr);
    	  HashMap<String, Object> toUser = (HashMap<String, Object>)vc.getValue(); 
        /*stmt = conn.prepareStatement("SELECT nickname FROM users WHERE id=?");
        stmt.setInt(1, toId.intValue());
        urs = stmt.executeQuery();*/
        if (!toUser.isEmpty())
          toName = (String)toUser.get("nickname");
        //stmt.close();
      }
      catch (Exception e)
      {
      ...
      try
      {
    	  ...
    	  HashMap<String, Object> item =  (HashMap<String, Object>)v.getValue();
      ...
      sp.printHTML(
        "<center><h2>Give feedback about your experience with "
          ...
          + "</h2><br>");
      sp.printHTML(
        "<form action=\"edu.rice.rubis.servlets.StoreComment\" method=POST>"
